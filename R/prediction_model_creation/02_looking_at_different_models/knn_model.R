...
get.predictions <- function(k, all.data, train.filter) {
  
  covariates <- 2:length(names(all.data))
  predictions <- list()
  predictions <- knn(k=k, all.data[train.filter,], all.data[!train.filter,], all.data[train.filter,][,1])
}


run.knn.model <- function(k) {
  ...
  model <- multinom(k, all.data[train.filter,])
