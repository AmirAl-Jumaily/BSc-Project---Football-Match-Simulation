...
get.predictions <- function(model, all.data, train.filter) {
  ...
  predictions$train   <- predict(model, type = "response")[,1]
  predictions$test    <- predict(model, all.data[!train.filter,covariates], type = "response")[,1]
...
run.nbinom.model <- function(mod.formula) {
  ...
  model <- glm.nb(mod.formula, all.data[train.filter,])
