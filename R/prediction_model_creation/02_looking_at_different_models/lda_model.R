...
get.predictions <- function(model, all.data, train.filter) {
  ...
  predictions$train   <- predict(model, all.data[train.filter,covariates])
  predictions$test    <- predict(model, all.data[!train.filter,covariates])
}


run.lda.model <- function(mod.formula) {
  ...
  model <- lda(mod.formula, all.train.data)
